...
class Classifier(nn.Module):
    def __init__(self, out_dim=2, noise=0.1, data=None):
        ...
        self.b = torch.randn(2000, 1, device=device) *0.02
    ...
    def transform_image(self, x):
        """
        Apply random Gaussian blur and random resizing to a batch of tensor images.
        Args:
            x: Tensor of shape (N, C, H, W), values in [0, 1]
        Returns:
            Tensor of shape (N, C, target_H, target_W)
        """
        
        # Random Gaussian blur sigma
        sigma = random.uniform(*self.blur_sigma_range)
        x = TF.gaussian_blur(x, kernel_size=self.blur_kernel, sigma=sigma)

        # Random scaling factor for resizing
        scale = random.uniform(*self.scale_range)
        scaled_size = [int(s * scale) for s in self.target_size]

        # Resize with random scale
        x = F.interpolate(x, size=scaled_size, mode='bilinear', align_corners=False)
    ...
    def forward(self, x):
        ...
        x1 = self.transform_image(x)
        x2 = self.transform_image(x)
...
if True:  # MNIST
    ...
    # Load MNIST dataset and filter for digits 0, 1, and 4
    ...
    # Select only digits 0, 1, and 4
    target_digits = [0, 1, 2]
    filtered_indices = [i for i, t in enumerate(mnist.targets) if t in target_digits]

    filtered_data = mnist.data[filtered_indices]
    filtered_targets = mnist.targets[filtered_indices]

    # Map original labels to 0, 1, 2 for 0, 1, 4
    label_map = {0: 0, 1: 1, 2:2}
    filtered_targets = torch.tensor([label_map[int(t)] for t in filtered_targets], dtype=torch.long)
    ...
    for digit in target_digits:
        idx = (filtered_targets == label_map[digit]).nonzero(as_tuple=True)[0][:N // len(target_digits)]
        imgs = filtered_data[idx].float() / 255.0  # Normalize to [0,1]
        imgs = imgs.view(imgs.size(0), -1)         # Flatten to (N, 784)
        val_circle.append(imgs.to(device))
        labels.append(filtered_targets[idx].to(device))


    val_circle = torch.cat(val_circle, dim=0)
    labels = torch.cat(labels, dim=0)

    print(val_circle.shape)
...
# Get first 24 images and labels
images = val_circle[:24].cpu().view(-1, 28, 28)  # (24, 28, 28)
image_labels = labels[:24].cpu()                # (24,)

# Create 4x6 grid
fig, axes = plt.subplots(4, 6, figsize=(10, 7))
for i, ax in enumerate(axes.flat):
    ax.imshow(images[i], cmap='gray')
    ax.set_title(f"Label: {image_labels[i].item()}", fontsize=10)
    ax.axis('off')

plt.tight_layout()
plt.savefig('val_circle_grid_with_labels.png')
plt.close()
